...
void draw_floor(Shader& shader, int model_id, int view_id, int projection_id) {
...
void draw_cubes(Shader& shader, int model_id, int view_id, int projection_id) {
  for(int i = 0; i < 2; ++i) {
    glm::mat4 model = glm::mat4(1.0f);
    glm::mat4 view = glm::mat4(1.0f);
    glm::mat4 projection = glm::mat4(1.0f);
...
void draw_larger_cubes(Shader& shader, int model_id, int view_id, int projection_id) {
  for(int i = 0; i < 2; ++i) {
    glm::mat4 model = glm::mat4(1.0f);
    glm::mat4 view = glm::mat4(1.0f);
    glm::mat4 projection = glm::mat4(1.0f);
...
int chp_23_stencil()
{
  ...
  GLFWwindow* window = init("Chapter 23: Stencil");

  if(!window) {
    return -1;
  }

  Shader shader("../shaders/chapter_23/vertex.vert", "../shaders/chapter_23/fragment.frag");
  Shader single_color("../shaders/chapter_23/vertex.vert", "../shaders/chapter_23/singleColor.frag");
  ...
  while(!glfwWindowShouldClose(window)) {
    ...
    // 0 - Cube, 1 - Cube, 2 - Floor
    {
      glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE);
      glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT);
      glStencilMask(0x00); // don’t update stencil buffer while drawing the floor
      shader.use();
      draw_floor(shader, model_id, view_id, projection_id);
    }

    {
      glStencilFunc(GL_ALWAYS, 1, 0xFF);
      glStencilMask(0xFF);
      draw_cubes(shader, model_id, view_id, projection_id);
    }

    {
      model_id = glGetUniformLocation(single_color.get_program(), "model");
      view_id = glGetUniformLocation(single_color.get_program(), "view");
      projection_id = glGetUniformLocation(single_color.get_program(), "projection");
      ...
      draw_larger_cubes(single_color, model_id, view_id, projection_id);
