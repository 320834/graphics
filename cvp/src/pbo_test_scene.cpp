  ...
  : SceneInterface(engine, scene_name)
{
  ...
  m_canvas->SetTexture(m_texture_id);
}

void PboTestScene::render() {
  m_video >> m_frame;
...
void PboTestScene::controls() {
  
}

void PboTestScene::setup_pbo(const cv::Mat& frame) {
...
void PboTestScene::update_buffer(void* dst) {
  if(!dst) {
    return;
  }
  ...
  for(int row = 0; row < m_frame.rows; ++row) {
    for(int col = 0; col < m_frame.cols; ++col) {
      const cv::Vec3b pix = m_frame.at<cv::Vec3b>(row, col);
...
void PboTestScene::update_pbo() {
  const int width = m_frame.cols;
  const int height = m_frame.rows;
  const int size_bytes =
    m_frame.total() * (m_frame.elemSize() + 1);

  m_pbo_index = (m_pbo_index + 1) % 2;
  int next_index = (m_pbo_index + 1) % 2;

  glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
  glBindTexture(GL_TEXTURE_2D, m_texture_id);
  glBindBuffer(GL_PIXEL_UNPACK_BUFFER, m_PBO[m_pbo_index]);
  ...
  glBindBuffer(GL_PIXEL_UNPACK_BUFFER, m_PBO[next_index]);
...
void PboTestScene::print_error() {
  switch (m_error) {
    ...
    case GL_STACK_OVERFLOW:
        ...
        break;
    default:
        std::cout << "Unknown OpenGL error: 0x" << std::hex << m_error << std::dec << '\n';
        break;
  }
}